...
public class BosqueTemplado {
    private Stage stage;
    private EcosistemasController controlador;

    // Constructor que recibe el Stage y el controlador
    public BosqueTemplado(Stage stage, EcosistemasController controlador) {
        this.stage = stage;
    ...
    public void start() {
        ...
        Label nombre = new Label("Bosque Templado");
        ...
        String descripcionTexto = model.obtenerDescripcionEcosistema("Bosque Templado");
        descripcion.setText(descripcionTexto);

        // Imagen Principal del Ecosistema
        ImageView img_1 = crearImageView("file:imagenes/ecosistema_2.png", 400, 400);
        grid.add(img_1, 0, 2);

        // Gráfico del Ecosistema
        view.CreaGraficaEcosistema grafica = new view.CreaGraficaEcosistema();
        LineChart<String, Number> grafico = grafica.crearGrafico(2);
        grid.add(grafico, 1, 3, 3, 1);

        // Crear la escena y mostrarla
        Scene scene = new Scene(grid, 1080, 600);
        stage.setScene(scene);
        stage.setTitle("Ventana Ecosistema - Bosque Templado");
        stage.show();
    }
